...
namespace db
{
	...
	int DBConnetor::StartRun(int id,Callback callback, BeginCallback bcallback)
	{
		...
		int err = mysql->ConnectMySql(m_mysqlcfg->ip,
										...
										m_mysqlcfg->dbname,
										m_mysqlcfg->port) ? 0 : -1;
		if (err != 0) LOG_MESSAGE("mysql start failed...%d:%s-%s\n", id, m_mysqlcfg->ip,m_mysqlcfg->dbname);
		else  LOG_MESSAGE("mysql connect success...%d:%s-%s\n", id, m_mysqlcfg->ip, m_mysqlcfg->dbname);

		//开启线程
		m_BuffsPool.Init(200, 200 * 1024);
	...
	void DBConnetor::Run()
	{
		...
		// 读线程0的m_BeginCallback被绑定到读取账户信息上
