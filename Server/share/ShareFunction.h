...
#define CMD_PLAYER_LOGIN			1000
#define CMD_PLAYER_MOVE				2000
#define CMD_PLAYER_PLAYERDATA		3000
#define CMD_PLAYER_LEAVE			4000
...
namespace share
{
	...
	// 字符串分割函数
	inline std::vector<std::string> split(std::string str, std::string patten, bool isadd = false)
	{
	...
	inline void serverTypeToString(int sid, std::string& str)
	{
		...
		else if (sid >= 2000 && sid < 3000) str = "Center";
	...
	{
		srand(time(NULL));
		u32 rode = rand() % 1000 + 100;
		char key[30];
		char md5[50];
		sprintf_s(key, "%s_%d", func::__ServerInfo->SafeCode, rode);
		memset(md5, 0, 50);
