...
#ifdef ___WIN32_
		//1、初始化路径 win32_AP，将IFileExePath赋值为.exe的路径
...
namespace net
{
	...
	{
		recvBuf = new char[func::__ServerInfo->RecvMax];
	...
	void S_CLIENT_BASE::Reset()
	{
		...
		memset(recvBuf, 0, func::__ServerInfo->RecvMax);
	...
	void S_SERVER_BASE::init(int sid)
	{
		...
		recvBuf = new char[func::__ClientInfo->RecvMax];
		sendBuf = new char[func::__ClientInfo->SendMax];
		recvBuf_Temp = new char[func::__ClientInfo->RecvOne];
	...
	void S_SERVER_BASE::reset()
	{
		...
		memset(recvBuf, 0, func::__ClientInfo->RecvMax);
		memset(sendBuf, 0, func::__ClientInfo->SendMax);
		memset(recvBuf_Temp, 0, func::__ClientInfo->RecvOne);
	}
}