...
namespace net
{
	...
	void TcpClient::runClient(u32 sid, char* ip, int port)
	{
		...
		if (err < 0)
		{
			LOG_MESSAGE("--------------client is error:%d...", err);
			return;
		}
	...
	bool TcpClient::setNonblockingSocket()
	{
		...
		{
			LOG_MESSAGE("--------------client errorcode is error..%d line:%d", errorcode, __LINE__);
			return -1;
		}
#else
...
//(4)：关闭套接字，关闭加载的套接字库(closesocket() / WSACleanup())。
	...
	s32 TcpClient::initSocket()
	{
		...
		{
			int fd = socketfd;
			LOG_MESSAGE("--------------client socket is error..%d line:%d", fd, __LINE__);
			return -1;
		}
		//2、设置套接字非阻塞模式
		setNonblockingSocket();
		//3、设置发送接受缓冲区
		int rece = func::__ClientInfo->RecvOne;
	...
	void TcpClient::connect_Select()
	{
		...
		// 这一次调用connectServer必定会成功,因为他已经连接成功，再此connect也必定value = 0;
	...
	bool TcpClient::connectServer()
	{
		...
		if (value == 0)//连接成功
		{
			...
			//LOG_MESSAGE("--------------client connect success...%d-%d\n", m_data.serverID, m_data.port);
			return true;
		}
		//处理连接错误
		if (value < 0)
		{
			...
			if (WSAEINVAL == err)
			{
				LOG_MESSAGE("--------------client 指定了一个无效参数.%d-%d\n", m_data.serverID, m_data.port);
				return false;
			}
			//意思是这个操作不能马上完成，正在连接中
			...
			{
				m_data.state = func::C_CONNECT;
				//LOG_MESSAGE("--------------client connect success2...%d-%d\n", m_data.serverID, m_data.port);
...
#else
			//LOG_MESSAGE("connect socket error:%d \n", errno);
	...
	void TcpClient::disconnectServer(const s32 errcode, const char* err)
	{
		...
		m_data.reset();
		initSocket();
		//LOG_MESSAGE("---------client disconnectServer %d-%s \n", errcode, err);
	...
	void  TcpClient::onAutoConnect()
	{
		...
		if (tempTime >= func::__ClientInfo->AutoTime)
		{
			...
			int fd = socketfd;
			LOG_MESSAGE("----------client autoConnect...%d threadid:%d \n", fd, getData()->ID);
			connectServer();
	...
	{
		auto c = this->getData();
		memset(c->recvBuf_Temp, 0, func::__ClientInfo->RecvOne);
		int receBytes = recv(socketfd, c->recvBuf_Temp, func::__ClientInfo->RecvOne, 0);
		if (receBytes > 0)
		{
			//保存数据
	...
	int TcpClient::onSaveData(int recvBytes)
	{
		...
		if (c->recv_Tail + recvBytes >= func::__ClientInfo->RecvMax)
		{
