...
#pragma pack(1)
	...
	struct S_CLIENT_BASE
	{
		...
		char            ip[MAX_IP_LEN];		// 记录连接客户端的IP地址
		u16			    port;
		int				ID;					// 对应索引号(在连接用户的vector中的下标)
		u8              rCode;

		//游戏记录数据
		s32             clientID;			// 各个功能服务器ID 
		u8              clientType;			// 0-玩家 1-DB 2-Center 3-Game 4-Gate 5-Login

		//接收数据
		bool			is_RecvCompleted;	// 接收的数据是否接收完成
		char*           recvBuf;			// recvmax，服务端接收来自这个客户端的缓冲区
		...
		//发送数据
		bool			is_Sending;			// 是否正在封包
		bool            is_SendCompleted;	// 当前是否发送完毕
		char*           sendBuf;			// sendmax，服务端对客户端的发送的缓冲区
		s32				send_Head;
		s32				send_Tail;
		s32				send_TempTail;
	...
	struct S_SERVER_BASE
	{
		...
		char*           recvBuf_Temp;		// 用来接收单次数据的
		char*           recvBuf;			// 接受池， 用来存储的
		s32				recv_Head;//头 消费者使用
		s32				recv_Tail;//尾 生产者使用
		s32				recv_TempHead;//临时头
		s32				recv_TempTail;//临时尾
...
#pragma pack(pop, packing)
	...
	public:
		...
		virtual void  registerCommands(int cmd, void* container) = 0;
	};
	...
	public:
		...
		virtual void  setOnSecureConnect(TCPCLIENTNOTIFY_EVENT event) = 0;
