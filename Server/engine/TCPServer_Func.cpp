...
void TCPServer::parseCommand()
{
	for (s32 i = 0; i < Linkers->length; i++)
	{
		...
		// 定期查看该用户的发送缓冲区内是不是有了数据，有了数据就popSendContext(用来作为判断是否发出)，然后发送数据
		this->postSend(c);
	}
}
...
{
	// 初始化为false,接收数据放入用户的缓存完成后设置为true,这样就能执行解析命令了
...
int TCPServer::setHeartCheck(SOCKET s)
{
	...
	if (dwError == SOCKET_ERROR)
	{
		...
		LOG_MESSAGE("SetHeartCheck->WSAIoctl()发生错误,错误代码: %ld  \n", dwError);
		return -1;
	}
	return 0;
}
